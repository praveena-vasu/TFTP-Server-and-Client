    ...
    if(mode == 1)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                printf("Sending DATA block %d\n", temp.body.data_packet.block_number);memset(&pack,0,sizeof(pack));
                //rec ack
                recvfrom(sockfd,&pack,sizeof(pack),0,(struct sockaddr*)&client_addr,&client_len);
                printf("Received opcode = %d, ACK block = %d, expected = %d\n",pack.opcode,pack.body.ack_packet.block_number, pack_num);
...
void receive_file(int sockfd, struct sockaddr_in client_addr, socklen_t client_len, char *filename) 
{
    ...
    if(mode == 1 || mode == 3)
    {
        ...
        do
        {
            ...
            //write data to file
            //bytes = write(fd,pack.body.data_packet.data,512);
